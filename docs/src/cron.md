...
specify one of its own, so a schedule that runs in a single timezone says so
once. An entry with its own `timezone` overrides it, which is how one schedule
can hold entries in several zones.
